...
// Default gifts data
const defaultEventGifts: EventGifts = {
  birthday: [
    {
      id: 1,
  ...
  justbecause: [
    {
      ...
      description:
        "This is a ticket to a future Harry Styles concert, an electrifying and unforgettable night of music. Imagine this: November 15, 2026, at 8:00 PM, at the iconic Madison Square Garden in New York City.",
    },
  ],
  christmas: [],
  valentine: [],
...
const initialState: GiftState = {
  eventGifts: loadFromLocalStorage("eventGifts", defaultEventGifts),
  selectedEvent: loadFromLocalStorage<SelectedEvent | null>(
    "selectedEvent",
    null
  ),
  selectedGifts: loadFromLocalStorage<Gift[]>("selectedGifts", []),
};
...
const giftSlice = createSlice({
  ...
  reducers: {
    ...
    setSelectedEvent: (state, action: PayloadAction<SelectedEvent>) => {
      ...
      saveToLocalStorage("selectedEvent", action.payload);
    },

    // Clear selected event
    clearSelectedEvent: (state) => {
      state.selectedEvent = null;
      saveToLocalStorage("selectedEvent", null);
    },

    // Add a gift to selected gifts
    addSelectedGift: (state, action: PayloadAction<Gift>) => {
      const existingIndex = state.selectedGifts.findIndex(
        (gift) => gift.id === action.payload.id
      );
      if (existingIndex === -1) {
        state.selectedGifts.push(action.payload);
        saveToLocalStorage("selectedGifts", state.selectedGifts);
      }
    },

    // Remove a gift from selected gifts
    removeSelectedGift: (state, action: PayloadAction<number>) => {
      state.selectedGifts = state.selectedGifts.filter(
        (gift) => gift.id !== action.payload
      );
      saveToLocalStorage("selectedGifts", state.selectedGifts);
    },

    // Clear all selected gifts
    clearSelectedGifts: (state) => {
      state.selectedGifts = [];
      saveToLocalStorage("selectedGifts", []);
    },

    // Set selected gifts (replace all)
    setSelectedGifts: (state, action: PayloadAction<Gift[]>) => {
      state.selectedGifts = action.payload;
      saveToLocalStorage("selectedGifts", action.payload);
    },

    // Add a new gift to an event category
    addGiftToEvent: (
      state,
      action: PayloadAction<{ eventKey: string; gift: Gift }>
    ) => {
      const { eventKey, gift } = action.payload;
      if (!state.eventGifts[eventKey]) {
        state.eventGifts[eventKey] = [];
      }

      const existingIndex = state.eventGifts[eventKey].findIndex(
        (g) => g.id === gift.id
      );
      if (existingIndex === -1) {
        state.eventGifts[eventKey].push(gift);
        saveToLocalStorage("eventGifts", state.eventGifts);
      }
    },

    // Remove a gift from an event category
    removeGiftFromEvent: (
      state,
      action: PayloadAction<{ eventKey: string; giftId: number }>
    ) => {
      const { eventKey, giftId } = action.payload;
      if (state.eventGifts[eventKey]) {
        state.eventGifts[eventKey] = state.eventGifts[eventKey].filter(
          (gift) => gift.id !== giftId
        );
        saveToLocalStorage("eventGifts", state.eventGifts);
      }
    ...
    // Update a gift in an event category
    updateGiftInEvent: (
      state,
      action: PayloadAction<{ eventKey: string; gift: Gift }>
    ) => {
      const { eventKey, gift } = action.payload;
      if (state.eventGifts[eventKey]) {
        const index = state.eventGifts[eventKey].findIndex(
          (g) => g.id === gift.id
        );
        if (index !== -1) {
          state.eventGifts[eventKey][index] = gift;
          saveToLocalStorage("eventGifts", state.eventGifts);
        }
      }
    },

    // Reset event gifts to default
    resetEventGifts: (state) => {
      state.eventGifts = defaultEventGifts;
      saveToLocalStorage("eventGifts", defaultEventGifts);
    },
...
export type { Gift, EventGifts, SelectedEvent, GiftState };
